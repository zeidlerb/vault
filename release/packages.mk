...
PACKAGES_META_DIR := .tmp/meta
PACKAGES_WITH_META_DIR := .tmp/packages-meta
...
PACKAGES := $(addprefix $(PACKAGES_DIR)/,$(addsuffix .yml,$(PKG_INDEXES)))
PACKAGES_META := $(addprefix $(PACKAGES_META_DIR)/,$(addsuffix .json,$(PKG_INDEXES)))
PACKAGES_WITH_META := $(addprefix $(PACKAGES_WITH_META_DIR)/,$(addsuffix .json,$(PKG_INDEXES)))
...
$(PACKAGES_DIR)/%.yml: $(DEFAULTS_DIR)/%.json $(RENDERED_DIR)/%.json
...
$(DOCKERFILES_DIR)/%: $(PACKAGES_DIR)/%.yml $(LAYER_TEMPLATES)
	@rm -rf $@; mkdir -p $@
...
$(PACKAGES_META_DIR)/%.json: $(PACKAGES_DIR)/%.yml $(DOCKERFILES_DIR)/%
...
$(PACKAGES_WITH_META_DIR)/%.json: $(PACKAGES_DIR)/%.yml $(PACKAGES_META_DIR)/%.json
	@yq -s '{ inputs: .[0], meta: .[1] }' $^ | yq -y . >> $@
...
$(LIST): $(PACKAGES_WITH_META) .tmp/layer-info.yml
	@cat .tmp/layer-info.yml > $@
	@yq -s '{ packages: . }' $(PACKAGES_WITH_META) | yq -y . >> $@
...
$(BUILD_ENV_DIR)/%.env: $(PACKAGES_META_DIR)/%.json $(LOCK)
