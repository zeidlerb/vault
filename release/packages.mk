...
# DOCKERFILES are generated by this rule.
$(DOCKERFILES_DIR)/%: $(PACKAGES_DIR)/%.json $(LAYER_TEMPLATES)
	...
	for NAME in $(LAYER_NAMES); do \
		...
		echo "Set BASE_LAYER_CHECKSUM ready for the next layer." > /dev/null; \
		BASE_LAYER_CHECKSUM=$$(sha256sum < $$DF | cut -d' ' -f1); \
		echo "Write this checksum to a file for later reference." > /dev/null; \
		echo "$$BASE_LAYER_CHECKSUM" > $$DF.checksum; \
		echo "Copy this layer out to layers.lock for reference from packages." > /dev/null; \
		mkdir -p layers.lock/$$NAME; \
		cp $$DF layers.lock/$$NAME/$$BASE_LAYER_CHECKSUM.Dockerfile; \
	done
