...
CI_COMMANDS_TPL     := .circleci/config/commands/@load-package.yml.tpl
CI_COMMANDS         := .circleci/config/commands/@load-package.yml
PACKAGE_SPEC        := release/packages.lock/pkgs.yml

.PHONY: ci-update-release-packages $(CI_WORKFLOW) $(CI_COMMANDS)
ci-update-release-packages: $(CI_WORKFLOW) $(CI_COMMANDS)
	@echo $^
...
$(CI_COMMANDS): $(PACKAGE_SPEC) $(CI_COMMANDS_TPL)
	@echo "==> Updating $@ to match $<"
	@echo "### GENERATED BY $@; DO NOT MANUALLY EDIT OR MERGE THIS FILE" > $@
	@cat $< | gomplate -f $(CI_COMMANDS_TPL) -d 'package-list=stdin://?type=application/yaml' >> $@
...
build-ci:
	...
	@[ -n "$(BUILD_PRODUCT_REVISION)" ] || { echo "You must set BUILD_PRODUCT_REVISION"; exit 1; }
